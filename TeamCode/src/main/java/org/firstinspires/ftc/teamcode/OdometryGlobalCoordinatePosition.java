...
class OdometryGlobalCoordinatePosition implements Runnable {
     ...
     *
     * @param verticalEncoderLeft  left odometry encoder, facing the vertical
     *                             direction
     * @param verticalEncoderRight right odometry encoder, facing the vertical
     *                             direction
     * @param horizontalEncoder    horizontal odometry encoder, perpendicular to the
     *                             other two odometry encoder wheels
     * @param threadSleepDelay     delay in milliseconds for the
     *                             GlobalPositionUpdate thread (50-75 milliseconds
     *                             is suggested)
     */
    public OdometryGlobalCoordinatePosition(DcMotor verticalEncoderLeft, DcMotor verticalEncoderRight,
        ...
        robotEncoderWheelDistance = Double.parseDouble(ReadWriteFile.readFile(wheelBaseSeparationFile).trim())
                * COUNTS_PER_INCH;
        this.horizontalEncoderTickPerDegreeOffset = Double
                .parseDouble(ReadWriteFile.readFile(horizontalTickOffsetFile).trim());

    }

    public OdometryGlobalCoordinatePosition(DcMotor verticalEncoderLeft, DcMotor verticalEncoderRight,
                                            DcMotor horizontalEncoder, double COUNTS_PER_INCH, double startingX, double startingY,
                                            double startingOrientation, int threadSleepDelay) {
        this.verticalEncoderLeft = verticalEncoderLeft;
        this.verticalEncoderRight = verticalEncoderRight;
        this.horizontalEncoder = horizontalEncoder;
        sleepTime = threadSleepDelay;
