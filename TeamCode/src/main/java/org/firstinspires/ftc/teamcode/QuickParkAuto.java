...
@Autonomous (name= "QuickParkAuto", group= "None")
public class QuickParkAuto extends SkystoneVuforiaNew {
...
//        }
        ...
        if(autoState == INIT_STATE) {
            autoState = WAIT_STATE;
            lastAutoState = INIT_STATE;
        }
        if(autoState == WAIT_STATE) {
            if(lastAutoState == INIT_STATE) {
//                lFoundationator.setPosition(0);
//                rFoundationator.setPosition(foundationatorPosition);
                //up
                lastAutoState = WAIT_STATE;
                startTime = getRuntime();
            }
            else if (startTime > currentTime - 4){
                driveOutOfWay(globalPositionUpdate);
            }
            else {
        ...
        if(autoState == PARK_STATE) {
            if(lastAutoState == WAIT_STATE) {
                lastAutoState = PARK_STATE;
                startTime = getRuntime();
            }
            else if (startTime > currentTime - 17) {

            }
            else if (startTime > currentTime - 21 && startTime < currentTime - 17) {
                driveBackToStart();
            }
            else {
                autoState = PLINE_STATE;
            }
        }
        if(autoState == PLINE_STATE) {
            driveToPark(globalPositionUpdate);
        }
        checkOdometry();
    }
    ...
    public void driveToPark (OdometryGlobalCoordinatePosition position) {
        goToPositionMrK(9,69,.7, .5,0); //TODO make Y = -4 for other location
    }
    public void driveOutOfWay (OdometryGlobalCoordinatePosition position) {
        goToPositionMrK(57, 48, .7, .5, 0);
    }
    public void driveBackToStart() {
        goToPositionMrK(9, 48, .7, .5, 0);
    }
